...
public:
    ...
    */
    EdgeWeight bidirSearch(NodeID s, NodeID t) {
        ...
        if(DIJKSTRACH_debug) cout << "bidirSearch 1 " << endl;
        // initialization of the hwy search
        _finished[0] = false;
        _finished[1] = false;
	if(DIJKSTRACH_debug) cout << "bidirSearch 2 " << endl;
        _concludeHwySearch = false;
	if(DIJKSTRACH_debug) cout << "bidirSearch 3 " << endl;
        if (s != SPECIAL_NODEID) insertStartNode(0,s);
	if(DIJKSTRACH_debug) cout << "bidirSearch 3.5 " << endl;
        if (t != SPECIAL_NODEID) insertStartNode(1,t);
	if(DIJKSTRACH_debug) cout << "bidirSearch 4 " << endl;
        CurrentNode u;
        int searchID = 0;
	    ...
	    if(DIJKSTRACH_debug) cout << "bidirSearch 5.1 " << endl;
	    ...
	    if(DIJKSTRACH_debug) cout << "bidirSearch 5.2 " << endl;
            // *** deleteMin ***
	    ...
	    if(DIJKSTRACH_debug) cout << "bidirSearch 5.3 " << endl;
	    ...
	    if(DIJKSTRACH_debug) cout << "bidirSearch 5.4 " << endl;
	...
	if(DIJKSTRACH_debug) cout << "bidirSearch 6 " << endl;
        processUnsettledNodes(0);
        processUnsettledNodes(1);
	if(DIJKSTRACH_debug) cout << "bidirSearch 7 " << endl;
        return _upperBound;
    }
...
private:
    ...
    */
    ...
    void insertStartNode(int searchID, NodeID nodeID) {
       if(DIJKSTRACH_debug)  cout << "CurrentNode::insertStartNode 1 " << endl;
        NodeID index = insert(searchID, 0, nodeID);
	if(DIJKSTRACH_debug) cout << "CurrentNode::insertStartNode 2 " << endl;
        PQData& data = pqData(searchID, index);
	if(DIJKSTRACH_debug) cout << "CurrentNode::insertStartNode 3 " << endl;
        data.setStartNode();
	if(DIJKSTRACH_debug) cout << "CurrentNode::insertStartNode 4 " << endl;
    }
    ...
    */
    NodeID insert(int searchID, EdgeWeight dist, NodeID nodeID) {
      if(DIJKSTRACH_debug) cout << "CurrentNode::insert 1 " << endl;
      if(_graph == NULL) cout << "SOOOOOO WRONG" << endl;
        NodeID index = _graph->node(nodeID).pqElement();
	if(DIJKSTRACH_debug) cout << "CurrentNode::insert 2 " << endl;
        if (index == 0) {
            index = pqueue(searchID).insert(dist);
	    if(DIJKSTRACH_debug) cout << "CurrentNode::insert 3.1 " << endl;
            _graph->node(nodeID).pqElement(index);
	    if(DIJKSTRACH_debug) cout << "CurrentNode::insert 3.2 " << endl;
            if (searchDirections == 2) pqueue(1-searchID).insertDummy();
	    if(DIJKSTRACH_debug) cout << "CurrentNode::insert 3.3 " << endl;
        }
        else {
            pqueue(searchID).insert(dist, index);
	    if(DIJKSTRACH_debug) cout << "CurrentNode::insert 4.2 " << endl;
        }
        PQData& data = pqData(searchID, index);
	if(DIJKSTRACH_debug) cout << "CurrentNode::insert 5 " << endl;
        data.init(nodeID);
	if(DIJKSTRACH_debug) cout << "CurrentNode::insert 6 " << endl;
        if ( localSearch )
        {
            data.setTarget(_graph->node(nodeID).isTarget());
	    if(DIJKSTRACH_debug) cout << "CurrentNode::insert 7 " << endl;
        }
        return index;
    }
