...
The purpose of statistics extraction is to build a frequency table like table \ref{tab:freq}.
...
\lnl{10}
\ForAll{$\spath_{s,t} | (s,t) \in \chi-table, s \in V, t \in V, s \neq t$} 
{
...
\lnl{10}
\tcp{\emph{Split historical query data into two sets for generating statistics and $\Psi$ candidates}}
$\langle SP_{candidates}, SP_{statistics} \rangle \leftarrow Split({SP})$ 

\tcp{\emph{Initialially fill H}}
\ForAll{$\spath_{s,t} | (s,t) \in SP_{candidates}, s \in V, t \in V, s \neq t$} 
{
    H.push(S$(\chi, \spath_{s,t}, \Psi), \spath_{s,t}$) \;
}
...
\ForAll{$(R_s,R_t) | (R_s,R_t) \in R, R(R_s) \in V, R(R_t) \in V, (R(R_s),R(R_t)) \exists SP_{candidates}$} 
{
    generate candidate \;
}
