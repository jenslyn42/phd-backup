...
If a path $P_{S,T}: v_S,v_{S+1},\ldots,v_T$ is a \spath, then $\forall$ $(v_K,v_L) | v_K \in P_{S,T} \wedge v_L \in P_{S,T}$ there is a  \spath $P_{K,L}$ with start-/end-node in $v_K,v_L$ i.e. \spath D in figure \ref{fig:rxmap} consists of: \\
