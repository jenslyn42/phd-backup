...
void Oracle::checkCache(intPair query)
{
	...
	if(debugProtos) cout << "one, probstaticcache::checkCache! :cacheSize:" << (int) cache.size() <<"::"<< endl;
	...
	if(debugProtos) cout << "two, probstaticcache::checkCache! cacheHit: " << cacheHit << endl;
	if(!cacheHit)
	{
		if(debugProtos) cout << "three, probstaticcache::checkCache!" << endl;
		spResult = RoadGraph::mapObject(ts)->dijkstraSSSP(query.first, query.second);
		numDijkstraCalls++;
	}
	if(debugProtos) cout << "four, probstaticcache::checkCache!" << endl;
}
...
void Scache::checkCache(intPair query)
{
	...
	if(debugProtos) cout << "one1, scache::checkCache! :cacheSize:" << (int) cache.size() <<"::"<< endl;
	...
	if(debugProtos) cout << "four, scache::checkCache! cacheHit: " << cacheHit << endl;
	if(!cacheHit)
	{
		if(debugProtos) cout << "five, scache::checkCache!" << endl;
		spResult = RoadGraph::mapObject(ts)->dijkstraSSSP(query.first, query.second);

		numDijkstraCalls++;
	}
	if(debugProtos) cout << "six, scache::checkCache!" << endl;
}
...
void Scache::readScoredQueriesFromTrainFile(string fn)
{
	...
	if(debugProtos) cout << "one, scache::readScoredQueriesFromTrainFile! " << endl;
	//find all pairs of nodeids in the training set to have SP done for them. map nodeids to coordinates.
	if(trainingData.is_open())
	{
		if(debugProtos) cout << "two, scache::readScoredQueriesFromTrainFile! " << endl;
	...
	do{
		...
		if(debugProtos) cout << "scache::readScoredQueriesFromTrainFile! cacheSize: " << cache.size() <<":" << cacheUsed << " (" << mh.size() << ")" << endl;
	} while(cacheSize >= cacheUsed && !mh.empty());
